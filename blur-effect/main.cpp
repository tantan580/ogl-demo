...
//            (const GLvoid*)(2*sizeof(GLfloat)));
    ...
    createTexture("/data/home/tanfang2/Workspace/texture.jpg",tex0);
...
int main()
{
    ...
    ShaderManager shader_linear_vert;
    shader_linear_vert.buildShader(vs,GL_VERTEX_SHADER);
    shader_linear_vert.buildShader(linear_vert_fs,GL_FRAGMENT_SHADER);
    GLuint linear_vert_pro = shader_linear_vert.link();
    std::cout<<"shader_linear_vert"<<std::endl;

    ShaderManager shader_linear_horiz;
    shader_linear_horiz.buildShader(vs,GL_VERTEX_SHADER);
    shader_linear_horiz.buildShader(linear_horiz_fs,GL_FRAGMENT_SHADER);
    GLuint linear_horiz_pro = shader_linear_horiz.link();
     std::cout<<"shader_linear_horiz"<<std::endl;
    //set data
    setData();
...
//    GLfloat * offsets = &kernel[1];
    ...
    //linear_vert_pro
    glUseProgram(linear_vert_pro);
    auto* monitor = glfwGetPrimaryMonitor();
    auto* mode = glfwGetVideoMode(monitor);
    float resx =mode->width*0.25;
    float resy =mode->height*0.25;

    glUniform1i(glGetUniformLocation(linear_vert_pro,"image"),0);
    glUniform2f(glGetUniformLocation(linear_vert_pro, "resolution"),(GLfloat)resx, (GLfloat)resy);
    glUniform1fv(glGetUniformLocation(linear_vert_pro, "weight"), weights.size(), &weights[0]);
    glUniform1fv(glGetUniformLocation(linear_vert_pro, "offset"), offsets.size(), &offsets[0]);
    //linear_horiz_pro
    glUseProgram(linear_horiz_pro);
    glUniform1i(glGetUniformLocation(linear_horiz_pro,"image"),0);
    glUniform2f(glGetUniformLocation(linear_horiz_pro, "resolution"),(GLfloat)resx, (GLfloat)resy);
    glUniform1fv(glGetUniformLocation(linear_horiz_pro, "weight"), weights.size(), &weights[0]);
    glUniform1fv(glGetUniformLocation(linear_horiz_pro, "offset"), offsets.size(), &offsets[0]);
